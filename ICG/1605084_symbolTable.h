...
private:
    ...
    void set_symbol_value(string sym_value)
    {
...
public:
    ...
    ScopeTable(int capacity,int uid)
    {
        unique_id = uid;
        table_size = capacity;
...
public:
    ...
    void enterScope(int size)
    {
        ...
        ScopeTable *newScopeTable =new ScopeTable(size,id);
    ...
    int lookUpScopeTableID(string name)
    {
        ScopeTable *temp = current_scope_table;
        while(temp != NULL)
        {
            if(temp->chain_look_up(name))
            {
                //cout<<"Found in ScopeTable# "<<temp_id<<" at posisition "<<temp->hash_pos<<","<<temp->lookup_index<<endl;
                //cout<<"found"<<endl;
                return temp->getScopeTableID();
            }
            temp = temp->parentScope;
        }
        return -1000;
    }
    int getCurrentScopeTableID()
    {
        return current_scope_table->getScopeTableID();
    }
    int getNextScopeTableID()
    {
        return current_scope_table->getScopeTableID()+1;
    }
